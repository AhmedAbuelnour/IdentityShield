...
namespace IdentityShield.Application.Endpoints.AccessControl.ResetPassword.Commands
{
    public static class ResetPasswordByEmailCommand
    {
        ...
        public class Handler(ShieldUserManager _userManager) : IEndPointRequestHandler<Command>
        {
            public async Task<IResult> Handle(Command request, CancellationToken cancellationToken)
            {
                ...
                if (!await _userManager.VerifyUserTokenAsync(user, Constant.TokenProviders.OtpTokenProvider, Constant.Purposes.ForgotPassword, request.Request.OTP))
                {
                ...
                if (await _userManager.ResetPasswordAsync(user, request.Request.Password) is IdentityResult identityResult && !identityResult.Succeeded)
                {
                    return TypedResults.UnprocessableEntity(OperationResult.Failed(new OperationError
                    {
                        Code = "XIDN0001",
