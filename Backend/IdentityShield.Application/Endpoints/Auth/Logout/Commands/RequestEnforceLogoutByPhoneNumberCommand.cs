...
{
    public static class RequestEnforceLogoutByPhoneNumberCommand
    {
                             ...
                             INotificationService _notificationService) : IEndPointRequestHandler<Command>
        {
            public async Task<IResult> Handle(Command request, CancellationToken cancellationToken)
            {
                ...
                if (result.Succeeded)
                {
                    await _userManager.SendSMSNotificationAsync(user, Constant.Purposes.LogoutRequest, TimeSpan.FromHours(24));
