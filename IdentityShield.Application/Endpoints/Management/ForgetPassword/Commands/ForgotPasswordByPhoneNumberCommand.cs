...
namespace IdentityShield.Application.Endpoints.Management.ForgetPassword.Commands
{
    public static class ForgotPasswordByPhoneNumberCommand
    {
        ...
        public class Handler(ShieldUserManager _userManager, INotificationService _notificationService) : IEndPointRequestHandler<Command>
        {
            ...
            {
                ApplicationUser? user = await _userManager.FindByEmailAsync(request.Request.PhoneNumber);
                ...
                if (await _userManager.GetTokenCountAsync(user, Constant.Purposes.ForgotPassword, Constant.TokenTrackerProvider.PhoneNumber, TimeSpan.FromHours(24), cancellationToken) is int tokenCount && tokenCount > 5)
                {
                    return TypedResults.UnprocessableEntity(OperationResult.Failed(new OperationError
                    {
                        Code = "XIDN0012",
                ...
                if (await _userManager.AddTokenAsync(user, Constant.Purposes.ForgotPassword, token, Constant.TokenTrackerProvider.PhoneNumber, cancellationToken) > 0)
                {
                    // send otp
