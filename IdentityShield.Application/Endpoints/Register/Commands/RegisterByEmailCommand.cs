    ...
    public static class RegisterByEmailCommand
    {
                             ...
                             JwtTokenService _jwtTokenService) : IEndPointRequestHandler<Command>
        {
            ...
            public async Task<IResult> Handle(Command request, CancellationToken cancellationToken)
            {
                ...
                if (await _userManager.AddTokenAsync(user, Constant.Purposes.Register, token, Constant.TokenTrackerProvider.Email, cancellationToken) > 0)
                {
                    // send otp
                    await _notificationService.SendEmailAsync(user, "Register -تسجيل حساب جديد", token, cancellationToken);
                }
